...
BASEDIR='/root/autodl-tmp/data/prostate/'
...
class ProstateDataset(torch_data.Dataset):
    def __init__(self,  mode, transforms, base_dir, domains: list,  
                 idx_pct = [0.7, 0.1, 0.2],  tile_z_dim = 3, extern_norm_fn = None, filter_all_0=False):
    ...
    def __read_dataset(self):
        ...
        for _domain, _sample_list in self.sample_list.items():
            for scan_id, itm in _sample_list.items():
                ...
                # filter out all 0 samples or not
...
def get_training(modality, idx_pct = [0.7, 0.1, 0.2], tile_z_dim = 3, filter_all_0=False):
